...
contract Summ{
    ...
    function partialWithdraw(uint _amount) public {
      if( msg.sender == creator && creatorTakeItOrLeaveItGiven == true && creatorTakeItOrLeaveItRespondedTo == false){
      require(balance[creator] >= (_amount + penalty),"you dont have enough to pay the penalty and remove that amount");
      balance[creator] -= (_amount + penalty); 
      balance[summFoundation] += penalty;
      creator.transfer(_amount);
      creatorTakeItOrLeaveItRespondedTo = true; 
      } else if(msg.sender == opponent && receiverTakeOrLeaveRequestGiven == true && receiversTakeOrLeaveRespondedTo == false) {
        require(balance[opponent] >= (_amount + penalty), "you dont have enough to pay the penalty and remove that amount"); 
    ...
    function initiateSoftGiverOffer(uint _amount) public onlyCreator softRoundCurrentlyActive {
      ...
      require(currentSoftGiverOffer < totalSoftOffersCap, "reached cap on amount of soft offers");  
      require(balance[creator] >= _amount + bufferAmount, "you do not have enough funds to make such an offer");
      require(_amount > 0);
      currentSoftGiverOffer++; 
      require(currentSoftGiverOffer == currentSoftReceiverOffer || (currentSoftGiverOffer - 1) == currentSoftReceiverOffer, "you must wait until the opponent puts in their offer to initate an additonal offer"); 
    ...
    emit SoftOfferGiven(creator,currentSoftGiverOffer, false); 
     ...
     function initiateSoftReceiverOffer(uint _amount) public onlyOpponent softRoundCurrentlyActive {
      require(_amount > 0); 
      require(currentSoftReceiverOffer < totalSoftOffersCap, "reached cap on amount of soft offers");  
      currentSoftReceiverOffer++;
      require(currentSoftReceiverOffer == currentSoftGiverOffer || (currentSoftReceiverOffer - 1) == currentSoftGiverOffer, "you must wait until the creator puts in their offer to initate a new offer"); 
   ...
   function opponentAcceptSoftOffer() public payable onlyOpponent sameOfferNumberAndBothGivenUnresolved softRoundCurrentlyActive { 
       ...
       require(balance[creator] >= finalSoftOffer, "creator does not have enough funds"); 
   ...
   function declineSoftOffer() public sameOfferNumberAndBothGivenUnresolved {
    require(msg.sender == creator || msg.sender == opponent, "you are not the creator or opponent"); 
   ...
   function initiateFirmGiverOffer(uint _amount) public onlyCreator unresolved{
     require(currentFirmGiverOffer < totalFirmOffersCap, "reached cap on amount of firm offers"); 
     uint bufferAmount = _amount * firmRange / 200; 
     uint totalRequiredAmount = _amount + bufferAmount; 
     require(balance[msg.sender] >= totalRequiredAmount, "you do not have enough funds to make such an offer");
  ...
  function initiateFirmReceiverOffer(uint _amount) public onlyOpponent unresolved {
    ...
    require(balance[msg.sender] >= penalty, "you must deposit the penalty amount to present a firm offer");
    require(mediumNumber > 0, "The medium Number has not been set yet"); 
    require(currentFirmReceiverOffer <= totalFirmOffersCap, "reached cap on amount of firm offers");
  ...
  function processFirmOffer(uint _amount) private {
    require(balance[opponent] >= penalty, " The opponent must redeposit the penalty amount for a firm offer to process");
    require(balance[creator] >= _amount, "The creator does not have enough funds n the contract for the offer to process");
    ...
    function receiverTakeItOrLeaveItResponse(bool _accept) public onlyOpponent unresolved pastFirstFirmOffers{
      ...
      if(_accept == true){
      require(balance[creator] >= finalTakeItOrLeaveItAmount, "The contract creator has removed his funds and this offer is no longer availabel");
    ...
    function creatorTakeOrLeaveResponse(bool _accept) public onlyCreator unresolved pastFirstFirmOffers{
      ...
      if(_accept == true) {
        require(balance[creator] >= finalTakeOrLeaveRequestAmount, "you dont have enough funds to process this offer");
