...
contract SummFactory is Ownable{
    ...
    function createSumm(address payable _opponent, uint _softOffers, uint _firmOffers, uint _softRange, uint _firmRange, uint _penaltyPercent) public {
        address newSummTerms = address(new SummTerms(payable(msg.sender), _opponent, _softOffers, _firmOffers, _softRange, _firmRange, _penaltyPercent, summFoundation));
